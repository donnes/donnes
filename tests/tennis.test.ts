...
import { LOCAL_TENNIS, parseTennis, chooseShot } from "../src/lib/habitat/tennis.ts";
import { createTennisCoach } from "../src/lib/habitat/typesafe.ts";

test("policies parse and normalise; a requested style is a preference, recovery comes first", () => {
...
test("the coach: batched questions, one paid call shared, cache, budget, missing key, wet weather, malformed replies", async () => {
  let calls = 0, now = 0;
  const coach = createTennisCoach("test", 1, async (_url: unknown, options: any) => {
    calls++;
    const body = JSON.parse(options.body);
    assert.deepEqual(Object.keys(body.questions), ["balanced", "stretched", "tired"]);
    assert.ok(Object.values(body.questions).every((q: any) => q.type === "choice"));
