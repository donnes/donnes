...
export async function run({ parsePlan, fallback, CONTEXTS }, { createDirector }, { createEndpoint }) {
  ...
  const url = s => new URL(`https://example.com/api/habitat${s}`);
  ...
  console.log("Habitat checks passed: validation, weather, cache, concurrency, cooldown, budgets, failures, endpoint, no-key mode.");
}
